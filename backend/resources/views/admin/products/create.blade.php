...
<head>
    ...
    <title>Create Product - MIX CURRY ADMIN</title>
...
<body class="bg-[#0F0F11] text-white antialiased font-sans flex flex-col min-h-screen">
    ...
    <main class="flex-grow max-w-4xl mx-auto px-10 py-12 w-full">
        <h1 class="text-4xl font-bold mb-10">Add New <span class="text-[#FF6600]">Product</span></h1>

        <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="bg-[#1A1A1E] border border-gray-800 rounded-[2rem] p-10 shadow-2xl grid grid-cols-1 md:grid-cols-2 gap-8">
            @csrf
            <div class="space-y-8">
                <div>
                    <label class="block text-[#FF6600] text-xs font-black uppercase tracking-widest mb-3">Product Name</label>
                    <input type="text" name="name" required placeholder="e.g. Spicy Curry" class="w-full bg-black/50 border border-gray-800 text-white rounded-2xl px-6 py-4 focus:outline-none focus:border-[#FF6600] transition font-bold">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="price" required placeholder="0.00" class="w-full bg-black/50 border border-gray-800 text-white rounded-2xl px-6 py-4 focus:outline-none focus:border-[#FF6600] transition font-bold">
            ...
            <div class="space-y-8">
                ...
                <div>
                    ...
                    <textarea name="desc" rows="4" placeholder="Describe this dish..." class="w-full bg-black/50 border border-gray-800 text-white rounded-2xl px-6 py-4 focus:outline-none focus:border-[#FF6600] transition text-gray-300"></textarea>
