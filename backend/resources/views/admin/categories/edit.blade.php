...
<head>
    ...
    <title>Edit Category - MIX CURRY ADMIN</title>
...
<body class="bg-[#0F0F11] text-white antialiased font-sans flex flex-col min-h-screen">
    ...
    <main class="flex-grow max-w-3xl mx-auto px-10 py-12 w-full">
        <h1 class="text-4xl font-bold mb-10">Edit <span class="text-[#FF6600]">Category</span></h1>

        <form action="{{ route('categories.update', $category->slug) }}" method="POST" class="bg-[#1A1A1E] border border-gray-800 rounded-[2rem] p-10 shadow-2xl space-y-8">
            @csrf
            @method('PUT')
            <div>
                <label class="block text-[#FF6600] text-xs font-black uppercase tracking-widest mb-3">Category Name</label>
                <input type="text" name="name" value="{{ $category->name }}" required placeholder="e.g. Italian Pasta" class="w-full bg-black/50 border border-gray-800 text-white rounded-2xl px-6 py-4 focus:outline-none focus:border-[#FF6600] transition text-lg font-bold">
            ...
            <div>
                ...
                <textarea name="desc" rows="4" placeholder="Brief description of this category..." class="w-full bg-black/50 border border-gray-800 text-white rounded-2xl px-6 py-4 focus:outline-none focus:border-[#FF6600] transition text-gray-300">{{ $category->desc }}</textarea>
