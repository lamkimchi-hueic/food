                        ...
                        <div className="space-y-6">
                            {cart.map((item, index) => (
                                <div key={index} className="flex items-center space-x-6 bg-[#141416] p-4 rounded-2xl border border-gray-800">
                                    <Link to={`/product/${item.slug}`} className="w-24 h-24 rounded-xl overflow-hidden bg-gray-900 border border-gray-800">
                                        <img src={item.img || getImageUrl(null)} className="w-full h-full object-cover" />
                                    </Link>
                                    <div className="flex-grow">
                                        <Link to={`/product/${item.slug}`}><h3 className="text-xl font-bold line-clamp-1">{item.name}</h3></Link>
