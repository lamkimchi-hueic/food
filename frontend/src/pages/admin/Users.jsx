    ...
    return (
        <div className="flex flex-col min-h-screen font-sans">
            ...
            <main className="flex-grow max-w-7xl mx-auto px-10 py-12 w-full">
                <h1 className="text-3xl font-bold mb-8">Manage <span className="text-[#FF6600]">Users</span></h1>
                ...
                <form onSubmit={handleSubmit} className="bg-[#1A1A1E] p-6 rounded-2xl border border-gray-800 mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <input type="text" placeholder="Username" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} required className="bg-[#0F0F11] border border-gray-800 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-[#FF6600] transition" />
                    <div className="relative">
                        <input type={showPassword ? "text" : "password"} placeholder={editId ? 'New password (leave blank to keep)' : 'Password'} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} {...(!editId ? { required: true } : {})} className="w-full bg-[#0F0F11] border border-gray-800 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-[#FF6600] transition pr-12" />
                    ...
                    {users.map((u) => (
                        <div key={u.id} className="bg-[#1A1A1E] p-5 rounded-2xl border border-gray-800 flex items-center justify-between">
                            ...
                            <div className="flex space-x-3">
                                ...
                                <button onClick={() => handleDelete(u.id)} className="px-4 py-2 bg-gray-800 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition text-sm font-bold">Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
